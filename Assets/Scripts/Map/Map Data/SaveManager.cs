...
public static class SaveManager
{
    ...
    public static bool CheckFileExists(string fileName, FileStreamType fileStreamType = FileStreamType.PlayerPref, bool fromPersistantDataPath = true, string pathFolder = "", string fileType = "txt")
    {
        ...
        switch (fileStreamType)
        {
            case FileStreamType.Binary:
            case FileStreamType.FileStream:
                string persistantDataPath = fromPersistantDataPath ? Application.persistentDataPath : Application.dataPath;
                string path = string.Concat(persistantDataPath, "/", pathFolder, fileName, ".", fileType);
                fileFound =  File.Exists(path);
                break;
            case FileStreamType.PlayerPref:
                fileFound = PlayerPrefs.HasKey(fileName);
                break;
            default:
                Debug.LogWarning($"SaveManager: File Stream Was Not Supported by Delete Function! {fileStreamType}");
                break;
        }
        return fileFound;
    }
    public static void DeleteFile(string fileName, string fileType, FileStreamType fileStreamType, string pathFolder, bool fromPersistantDataPath)
    {
        switch (fileStreamType)
        {
            case FileStreamType.Binary:
            case FileStreamType.FileStream:
                ...
                if (!File.Exists(path))
                {
                    Debug.Log("File does not exist in this path:\nPath: " + path);
                }
                else
                {
                    Debug.Log("File was found and got deleted\nPath: " + path);
                    File.Delete(path);
                }
