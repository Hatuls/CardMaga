...
namespace CardMaga.MetaData.Collection
{
    ...
    public class CardsCollectionDataHandler
    {
        ...
        {
            List<MetaCardInstanceInfo> cardDatas = metaAccountData.AccountCards.OrderBy(x => x.CoreID).ToList();
            ...
            foreach (var cardData in cardDatas)
            {
                ...
                {
                    _collectionCardDatas[cardData.CoreID].AddCardInstance(cardData);
                }
                else
                {
                    _collectionCardDatas.Add(cardData.CoreID,new MetaCollectionCardData(cardData));
                }
            }
        ...
        public CardsCollectionDataHandler GetCollectionCopy()
        {
            var cardDatas = new List<MetaCollectionCardData>(_collectionCardDatas.Values.Count);

            foreach (var value in _collectionCardDatas.Values)
            {
                cardDatas.Add(value.GetCopy());
            }

            return new CardsCollectionDataHandler(cardDatas);
        }
    }
}